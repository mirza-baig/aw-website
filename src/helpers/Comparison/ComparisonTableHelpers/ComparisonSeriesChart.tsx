...
type LegendRow =
  ...
  | { kind: 'row'; key: string; label?: string; fieldName: string };
...
const buildLegendRows = (
  ...
  tableConfigFields: Record<string, { value?: unknown } | undefined>
): LegendRow[] => {
...
/**
 * TEMPORARY MOCK DATA: `chartEyebrow` / `chartTitle` / `chartDescription` aren't
 * authored on the AW_ComparisonSeriesTable template in Sitecore yet (see the field
 * list from earlier — they still need to be added there and synced into
 * `.sitecore/AndersenWindows.model.ts`). These constants are only a fallback so the
 * legend header can be previewed before that happens.
 *
 * TO SWITCH TO REAL VALUES: once the fields exist in Sitecore and are authored,
 * `props.fields.chartEyebrow` etc. will be populated and the `??` fallback below
 * will stop being used automatically — no code change needed. Once every
 * datasource has real content authored, delete these MOCK_CHART_* constants and
 * the `?? MOCK_CHART_*` fallbacks so an unauthored field just renders blank again
 * instead of silently showing placeholder copy.
 */
const MOCK_CHART_EYEBROW: Field<string> = { value: 'EYEBROW' };
const MOCK_CHART_TITLE: Field<string> = { value: 'Title' };
const MOCK_CHART_DESCRIPTION: Field<string> = { value: 'Description' };

/**
 * TEMPORARY MOCK DATA: `finalColumnCTA` isn't authored on AW_ComparisonSeriesTable
 * yet either. Same pattern as MOCK_CHART_* above — fallback only, so the final
 * "Explore all Series" column can be previewed before the field exists in Sitecore.
 *
 * TO SWITCH TO REAL VALUES: once `finalColumnCTA` is added to the Sitecore
 * template and authored, `props.fields.finalColumnCTA` will be populated and this
 * fallback stops being used automatically. Delete MOCK_FINAL_COLUMN_CTA and its
 * `?? MOCK_FINAL_COLUMN_CTA` usage once every datasource has a real link authored.
 */
const MOCK_FINAL_COLUMN_CTA: LinkField = { value: { href: '#', text: 'Explore all Series' } };

type FinalRowCTA = { text: string; field: LinkField };

const getFinalRowCTAs = (items: Item[] | undefined): FinalRowCTA[] => {
  return (items ?? [])
    .map((item): FinalRowCTA | undefined => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const fields = item?.fields as Record<string, any> | undefined;
      const link = fields?.link as LinkField | undefined;
      if (!link?.value?.href) {
        return undefined;
      }
      const text =
        (fields?.linkText as Field<string> | undefined)?.value ||
        link.value.text ||
        item.displayName;
      return { text: text ?? '', field: link };
    })
    .filter((cta): cta is FinalRowCTA => !!cta);
};

/**
 * TEMPORARY MOCK DATA: `finalRowCTAOptions` (source template AW_ComparisonFinalRowCTA)
 * isn't authored on AW_ComparisonSeriesTable yet either — same pattern as
 * MOCK_CHART_* above. Fallback only, so the final CTA row below the table can be
 * previewed before the field/items exist in Sitecore.
 *
 * TO SWITCH TO REAL VALUES: once `finalRowCTAOptions` is added to the Sitecore
 * template and content authors select real AW_ComparisonFinalRowCTA items,
 * `props.fields.finalRowCTAOptions` will be populated and this fallback stops
 * being used automatically. Delete MOCK_FINAL_ROW_CTA_OPTIONS and its
 * `?? MOCK_FINAL_ROW_CTA_OPTIONS` usage once every datasource has real items authored.
 */
const MOCK_FINAL_ROW_CTA_OPTIONS: Item[] = [
  {
    name: 'mock-get-a-quote',
    displayName: 'Get a Quote',
    fields: { link: { value: { href: '#', text: 'Get a Quote' } } },
  },
  {
    name: 'mock-find-a-dealer',
    displayName: 'Find a Dealer',
    fields: { link: { value: { href: '#', text: 'Find a Dealer' } } },
  },
];
...
export const ComparisonSeriesChart = (props: ComparisonSeriesChartProps) => {
  ...
  const tableConfigFields = (props.fields.tableConfiguration?.fields ?? {}) as Record<
    string,
    { value?: unknown } | undefined
  >;
  const legendRows = buildLegendRows(props.fields.tableStructure, tableConfigFields);
  // TEMPORARY: falls back to MOCK_FINAL_ROW_CTA_OPTIONS — see the comment above it.
  const finalRowCTAs = getFinalRowCTAs(
    props.fields.finalRowCTAOptions ?? MOCK_FINAL_ROW_CTA_OPTIONS
  );

  console.log('ComparisonSeriesChart props.fields:', props.fields);
  console.log(
    'ComparisonSeriesChart comparisonObject:',
    legendRows,
    comparisonObject,
    seriesToCompare,
    finalRowCTAs
  ...
  return (
    <>
        ...
        <div className="relative">
          <div className={`flex ${isMobile ? 'gap-1 p-[10px]' : 'gap-5 py-[15px]'}`}>
            ...
            >
              ...
              >
                {(() => {
                  // TEMPORARY: falls back to MOCK_CHART_* until chartEyebrow/chartTitle/
                  // chartDescription exist in Sitecore — see the comment above MOCK_CHART_EYEBROW.
                  const chartEyebrow = props.fields.chartEyebrow ?? MOCK_CHART_EYEBROW;
                  const chartTitle = props.fields.chartTitle ?? MOCK_CHART_TITLE;
                  const chartDescription = props.fields.chartDescription ?? MOCK_CHART_DESCRIPTION;
                  return (
                    <>
                      {chartEyebrow.value && (
                        <span
                          className={`font-bold uppercase tracking-wider text-[#F26924] ${isMobile ? 'text-[11px] leading-tight' : 'text-sm'}`}
                      ...
                      {chartTitle.value && (
                        <span
                          className={`font-bold leading-tight line-clamp-3 ${isMobile ? 'text-[16px]' : 'text-xl'}`}
            ...
            <div className="relative flex-1 min-w-0">
              ...
              >
                ...
                {/* ── Final column CTA ── */}
                {/* TEMPORARY: falls back to MOCK_FINAL_COLUMN_CTA — see the comment above it. */}
                {(() => {
                  const finalColumnCTA = props.fields.finalColumnCTA ?? MOCK_FINAL_COLUMN_CTA;
                  return (
                    finalColumnCTA.value?.href && (
                      <div
                      ...
                      >
                        <LinkWrapper
                          field={finalColumnCTA}
                          ariaLabel={{ value: 'Explore all series' }}
                          className={`flex flex-col items-center justify-center gap-2 px-2 !font-sans font-normal  ${isMobile ? 'text-[10px]' : 'text-md'}`}
                        >
      ...
      {finalRowCTAs.length > 0 && (
        <div className="col-span-12 flex flex-wrap items-center justify-center gap-3 pt-l">
          {finalRowCTAs.map((cta) => (
            <LinkWrapper
              key={cta.text}
              field={cta.field}
              ariaLabel={{ value: cta.text }}
              className="rounded-full border-4 border-[#F26924] px-m py-[9px] text-center font-sans text-button font-heavy text-[#001722] no-underline hover:bg-[#F26924] hover:text-white"
            />
          ))}
        </div>
      )}
