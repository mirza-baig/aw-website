...
import { decodeHtml, encodeUrl, singleOrDefault } from './utils';
...
export class ViewModelBuilderBase {
  constructor(product, moduleData) {
    ...
    this._renoworksClient = new RenoworksClient(
      ...
      moduleData.renoworks.apiHost,
      moduleData.renoworks.prefix
    );
    this._productSettings = new Set();
    this._relatedProduct = singleOrDefault(
      moduleData.products,
      (_) => _.id === product.relatedProductId
    );
  ...
  buildSummary() {
    let relatedProduct =
      ...
      singleOrDefault(this.moduleData.products, (_) => _.id === this.product.relatedProductId);
    let summary = new SummaryViewModel(
      ...
      this.moduleData.findADealer,
      this.moduleData.requestAQuote,
      this.moduleData.favoritesLink,
  ...
  _updateProductSettingValuesFromRenoworksResult(productSettingValues, result, productSettings) {
    ...
    for (let returnedSetting of returnedSettings) {
      ...
      let productSetting = singleOrDefault(
        productSettings,
        ...
        if (productSetting.childKeys !== null) {
          ...
          if (valueParts.length > 1) {
            ...
            for (let value of valueParts) {
              ...
              let childKey = singleOrDefault(
                productSetting.childKeys,
...
export class SizingAttributeViewModel extends AttributeViewModelBase {
  constructor(sizingText, customSizingText, renoworksKeyName, productSide) {
