...
export async function getComponentServerProps(rendering: ComponentRendering) {
  const props = rendering as ComponentRendering<
    Sitecore.Forms.GenericFormBuilder.Fields.TradeTypeDropDown.TradeTypeDropDownField['fields']
  >;
  ...
  if (props.fields.showDefaultDisplay.value) {
    options.unshift({ id: 'default', label: props.fields.defaultDisplay.value, value: '' });
  }
