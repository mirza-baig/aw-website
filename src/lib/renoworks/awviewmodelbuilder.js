...
export class AWViewModelBuilder extends ViewModelBuilderBase {
    ...
    if (this._sizingIsEnabled) {
      ...
      if (EntryDoorProducts.includes(this.product.renoworksKey)) {
        if (productSettingValues.has(this._Configuration)) {
          this._currentProductConfiguration = this.product.configuration.singleOrDefault(
            (configuration) =>
  ...
  buildAttributesForRenoworksResult(renoworksResult) {
    ...
    // Hardware
    this._buildHardwareSection(renoworksResult, imageHelpers);
  ...
  _buildHardwareSection(renoworksResult, imageHelpers) {
    let manufacturerTab = renoworksResult.interiorResult.product.tab.singleOrDefault(
  ...
  _buildSideliteStyleAttribute(renoworksResult, imageHelpers, attributes) {
    let tab = renoworksResult.exteriorResult.product.tab.singleOrDefault(
  ...
  _buildSizingAttribute(renoworksResult, attributes) {
    ...
    if (this._currentProductConfiguration === null) {
      ...
      let vm = new SizingAttributeViewModel(DesignToolStepName.Sizing);
  ...
  _buildBottomRailHeightAttribute(renoworksResult, imageHelpers, attributes) {
    ...
    if (tab != null) {
      ...
      let vm = new TextAttributeViewModel(RenoworksKeys.BottomRailHeight.name);
  ...
  _buildRailAndStileWidthAttribute(renoworksResult, imageHelpers, attributes) {
    ...
    if (tab != null) {
      ...
      let vm = new TextAttributeViewModel(RenoworksKeys.RailAndStileWidth.name);
  ...
  _buildFrameStainAttribute(renoworksResult, imageHelpers, attributes) {
    ...
    const [, colorPart] = selectedInteriorColor.split('; ');
    ...
    if (tab != null) {
      let vm = new TabbedSwatchAttributeViewModel(RenoworksKeys.FrameStain.name);
  ...
  _buildInteriorStainPaintAttribute(renoworksResult, imageHelpers, attributes) {
    ...
    if (tab != null) {
      let vm = new TabbedSwatchAttributeViewModel(RenoworksKeys.InteriorStainPaint.name);
  ...
  _buildHardwareAttribute(renoworksResult, imageHelpers, attributes) {
    ...
    if (tab != null) {
      for (let component of tab.component) {
        ...
        if (component.color) {
          let optionalHardwareTab = renoworksResult.interiorResult.product.tab.singleOrDefault(
            (_) => _.name === RenoworksKeys.OptionalHardware.name
          );
          let optionalHardware =
            optionalHardwareTab === null
              ? null
              : optionalHardwareTab.component.singleOrDefault((_) => _.selected);
    ...
    tab = renoworksResult.interiorResult.product.tab.singleOrDefault(
      (_) => _.name === RenoworksKeys.OptionalHardware.name
    );
  ...
  _buildBrandHardwareFinishAttribute(renoworksResult, imageHelpers, attributes) {
    ...
    if (tab != null) {
      ...
      for (let option of vm.options) {
        showPvdNote = showPvdNote || pvdColors.includes(option.title);
      }
  ...
  _buildGrilleWidthAttribute(renoworksResult, imageHelpers, attributes) {
    let grilleStyle = renoworksResult.interiorResult.product.tab.singleOrDefault(
      (_) => _.name === RenoworksKeys.GrilleStyle.name
    );
    if (grilleStyle === null) {
      return;
    ...
    let selectedGrilleStyle = grilleStyle.component.singleOrDefault((_) => _.selected);
    if (selectedGrilleStyle === null) {
      return;
    }

    if (selectedGrilleStyle.grilleWidth && selectedGrilleStyle.grilleWidth.length > 0) {
      let vm = new TextAttributeViewModel(RenoworksKeys.GrilleStyle.name);
  ...
  _buildExteriorFrameColorAttribute(renoworksResult, imageHelpers, attributes) {
    ...
    let sashColorTab = renoworksResult.exteriorResult.product.tab.singleOrDefault(
  ...
  _buildExteriorTrimColorAttribute(renoworksResult, imageHelpers, attributes) {
    let exteriorTrimProfile = renoworksResult.exteriorResult.product.tab.singleOrDefault(
      (_) => _.name === RenoworksKeys.ExteriorTrimProfile.name
    );
    ...
    let selectedExteriorTrimProfile = exteriorTrimProfile.component.singleOrDefault(
      (_) => _.selected
    );
    ...
    if (
      ...
      let vm = new DependentSwatchAttributeViewModel(
        selectedControlOption,
        RenoworksKeys.ExteriorTrimProfile.name
      );
