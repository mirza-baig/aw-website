...
import { WhenIsFirstLevelChildOfCondition } from './when-is-first-level-child-of-condition';
...
describe('lib > rules > conditions > coveo > items > when-is-first-level-child-of-condition', () => {
  test('generates void node for invalid id', () => {
    ...
    const condition = new WhenIsFirstLevelChildOfCondition(element, context);
    // Act
  ...
  test('generates correct node for valid id', () => {
    ...
    expect(node.fieldName).toBe(CoveoFields.ParentId);
