...
import { WhenTemplateIsExactlyCondition } from './when-template-is-exactly-condition';

describe('lib > rules > conditions > coveo > items > when-is-first-level-child-of-condition', () => {
  test('generates void node for invalid id', () => {
    // Arrange
    const element = mock<XmlElement>();
    const context = mock<IConditionFactoryContext>();
    const condition = new WhenTemplateIsExactlyCondition(element, context);
    // Act
  ...
  test('generates correct node for valid id', () => {
    ...
    const element = mock<XmlElement>({ attributes: { templateid: uuid } });
    const context = mock<IConditionFactoryContext>();
    const condition = new WhenTemplateIsExactlyCondition(element, context);
    // Act
