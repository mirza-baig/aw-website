...
export class SearchQueryService {
   ...
   * @param {AWSitecoreClient} client that fetches data from a GraphQL endpoint.
   */
   ...
   */
  async fetchAllResults<T>(query: string, args: SearchQueryVariables): Promise<T[]> {
    let results: T[] = [];
    let hasNext = true;
    let after = '';

    while (hasNext) {
      const fetchResponse = await this.options.sitecoreClient.getData<SearchQueryResult<T>>(query, {
        ...args,
        after,
      });
   ...
   * @throws {RangeError} if the provided language(s) is(are) not valid.
   */
  async fetchPageInfo<T>(query: string, args: SearchQueryVariables): Promise<PageInfo[]> {
    ...
    while (hasNext) {
      ...
      pageInfo = pageInfo.concat(fetchResponse?.search?.pageInfo);
      hasNext = fetchResponse.search.pageInfo.hasNext;
      after = fetchResponse.search.pageInfo.endCursor;
    }
  ...
  async fetchPageResults<T>(
    ...
    return fetchResponse?.search?.results;
  }
}
