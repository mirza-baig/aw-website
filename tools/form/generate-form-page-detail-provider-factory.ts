...
import buildFactory, {
  FormPageDetailProviderObjectFile,
} from './templates/form-page-detail-provider-factory';
 ...
 */
export type GenerateFormPageDetailProviderFactoryConfig = {
  /**
...
const fileFormat = new RegExp(/(form-page-detail-provider)\.ts$/);
 ...
 */
export function generateFormPageDetailProviderFactory({
  rootPath,
  generatedFile,
  destinationPath,
}: GenerateFormPageDetailProviderFactoryConfig) {
  return async () => {
    ...
    console.log(`Writing form page detail provider factory to ${outputPath}`);
...
function getList(path: string): Array<FormPageDetailProviderObjectFile> {
  const items = getItems({
    ...
    cb: (item) => console.debug(`Registering form page detail provider ${item.fieldName}`),
